...
def process_image(img):
    #undistort the image
    dbfile = open('calibration_pickle.p', 'rb')  
    dist_pickle=pickle.load(dbfile)
    mtx=dist_pickle["mtx"]
    dist=dist_pickle["dist"]
    img = cv2.undistort(img,mtx,dist,None,mtx)
    src = np.array([[ 588.8,  446.4],[ 691.2,  446.4],[1126.4,  673.2],[ 153.6,  673.2]], dtype='float32')
...
#Output_video = 'output_challenge_video90.mp4'
#Input_video = 'challenge_video.mp4'


if __name__=="__main__":
    Output_video = 'output_challenge_video99.mp4'
    Input_video = 'project_video.mp4'
    clip1 = VideoFileClip(Input_video)
    video_clip = clip1.fl_image(process_image) # This function expects color images
    video_clip.write_videofile(Output_video,fps=24.0)#, audio=False)
